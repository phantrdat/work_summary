...
# COCO sample: http://images.cocodataset.org/val2017/000000039769.jpg

parser = argparse.ArgumentParser(description='ONNX EfficientDet')
parser.add_argument('--img', type=str, help='Input image')
parser.add_argument('--onnx_model', type=str, help='ONNX converted model')
parser.add_argument('--compound_coef', type=int, help='Compound coefficient')
parser.add_argument('--output', type=str, default='result', help='Saved files directory')
...
compound_coef = int(args.compound_coef)
...
with torch.no_grad():
    ...
    print(f'{args.output}/d{compound_coef}_out_onnx_'+ os.path.basename(img_path))
    display(f'{args.output}/d{compound_coef}_out_onnx_'+ os.path.basename(img_path) , out, ori_imgs, imshow=False, imwrite=True)

    t = time.time() - t_start
    # print(f'{np.array(onnx_times).mean()} seconds, {1 / np.array(onnx_times).mean()} FPS, @batch_size 1')
    print(f"Inference after {t} seconds")
